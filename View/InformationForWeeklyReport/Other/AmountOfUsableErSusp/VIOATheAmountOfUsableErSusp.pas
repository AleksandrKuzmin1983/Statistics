...
function TTheAmountOfUsableErSusp.GetGridDB(NameForm: TForm): TDBGrid;
  ...
  SQL:='SELECT Exped.Код, Exped.ДАТАЛЗ, Exped.НС, Sum(Exped.ПГЭС) AS [Sum-ПГЭС] ' +
  'FROM Exped GROUP BY Exped.ДАТАЛЗ, Exped.НС, Exped.Код HAVING (((Sum(Exped.ПГЭС))>0)) ORDER BY Exped.ДАТАЛЗ desc;';
