...
type
  ...
  private
    ...
    ResultEdit3: TEdit;
    form1: TForm;
    TempResult: TTheNumberOfBloodDonations;
  public
   ...
   // function GetStartCurrentValue(StartDateCal: TDateTimePicker): TDate;
...
begin
  if not Assigned(TempResult) then
    TempResult:=TTheNumberOfBloodDonations.create(GetCalendarStartDate(form1).Date, GetCalendarEndDate(form1).Date);
    ResultEdit1.Text:=TempResult.GetValue;
  TempResult:=nil;
end;
...
function TNumberOfDonations.GetCalendarEndDate(NameForm: TwinControl): TDateTimePicker;
begin
...
function TNumberOfDonations.GetCalendarStartDate(NameForm: TwinControl): TDateTimePicker;
begin
