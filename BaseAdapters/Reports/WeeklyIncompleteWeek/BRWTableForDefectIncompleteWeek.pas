unit BRWTableForDefectIncompleteWeek;
...
type
  IMRWTableForDefectIncompleteWeek = interface
  ...
  TMRWTableForDefectIncompleteWeek = class(TInterfacedObject,
    IMRWTableForDefectIncompleteWeek)
  private
    SQL: String;
...
procedure TMRWTableForDefectIncompleteWeek.destroy;
begin
  TempConnect.destroy;
  TempConnect:=nil;
...
procedure TMRWTableForDefectIncompleteWeek.GetContent;
var i: integer;
begin
  ...
  if (TempQueryErSusp.RecordCount + TempQueryPlasma.RecordCount + TempQueryWholeBlood.RecordCount)>3 then
  begin
    SetLength(ResultMass, TempQueryErSusp.RecordCount+TempQueryPlasma.RecordCount+TempQueryWholeBlood.RecordCount-3);
    ...
    begin
      ...
      ResultMass[i].TypeOfDefect:=VarToStr(CheckNull.CheckedValue(TempQueryErSusp.Fields[1].value));
      ResultMass[i].Volume:=VarToStr(FormatFloat('0.000',CheckNull.CheckedValue(TempQueryErSusp.Fields[2].value)));
      TempQueryErSusp.Next;
    end;

    TempQueryPlasma.Recordset.MoveFirst;
    TempQueryPlasma.Next;
    for i:=TempQueryErSusp.RecordCount-1 to TempQueryErSusp.RecordCount + TempQueryPlasma.RecordCount-3 do
    begin
      ResultMass[i].TypeOfProduct:=' (СЗП)';
      ResultMass[i].TypeOfDefect:=VarToStr(CheckNull.CheckedValue(TempQueryPlasma.Fields[1].value));
      ResultMass[i].Volume:=VarToStr(FormatFloat('0.000',CheckNull.CheckedValue(TempQueryPlasma.Fields[2].value)));
      TempQueryPlasma.Next;
    end;
    if Length(ResultMass)=TempQueryErSusp.RecordCount + TempQueryPlasma.RecordCount-2 then
    begin
      TempQueryErSusp.Close;
      TempQueryPlasma.Close;
      TempQueryWholeBlood.Close;
    ...
    for i:=TempQueryErSusp.RecordCount + TempQueryPlasma.RecordCount-2 to
      TempQueryErSusp.RecordCount + TempQueryPlasma.RecordCount + TempQueryWholeBlood.RecordCount-4 do
    begin
      ResultMass[i].TypeOfProduct:=' (цел. кр.)';
      ResultMass[i].TypeOfDefect:=VarToStr(CheckNull.CheckedValue(TempQueryWholeBlood.Fields[0].value));
      ResultMass[i].Volume:=VarToStr(FormatFloat('0.000',CheckNull.CheckedValue(TempQueryWholeBlood.Fields[1].value)));
      TempQueryWholeBlood.Next;
    end;
...
function TMRWTableForDefectIncompleteWeek.GetTypeOfDefect(i: integer): string;
begin
  result := ResultMass[i].TypeOfDefect;
end;

function TMRWTableForDefectIncompleteWeek.GetRowCount: integer;
begin
  result:=Length(ResultMass);
end;

function TMRWTableForDefectIncompleteWeek.GetVolume(i: integer): string;
begin
  result := ResultMass[i].Volume;
end;

function TMRWTableForDefectIncompleteWeek.GetTypeOfProduct(i: integer): string;
begin
