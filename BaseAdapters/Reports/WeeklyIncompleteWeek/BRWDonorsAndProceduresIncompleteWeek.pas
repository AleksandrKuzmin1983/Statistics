unit BRWDonorsAndProceduresIncompleteWeek;
...
type
  IBRWDonorsAndProceduresIncompleteWeek = interface
    function GetValue(i: integer): string;
  end;

  TBRWDonorsAndProceduresIncompleteWeek = class(TInterfacedObject,
    IBRWDonorsAndProceduresIncompleteWeek)
  private
...
constructor TBRWDonorsAndProceduresIncompleteWeek.create;
var
  i: integer;
begin
  ...
  CodeSite.Send(FormatDateTime('c', Now) + ' TBRWDonorsAndProceduresIncompleteWeek.create выполнена');
end;

function TBRWDonorsAndProceduresIncompleteWeek.GetValue(i: integer): string;
begin
  result := VarToStr(FormatFloat('0', TempArray[i]));

  CodeSite.Send(FormatDateTime('c', Now) + ' TBRWDonorsAndProceduresIncompleteWeek.GetValueLitr выполнена', result);
end;
