...
begin
  ...
  Try
    TempQuery.SQL.Add
      ('SELECT Sum(Tromb.КонТ), Sum(TrombComponents.КПЛТ), Sum(TrombBrak.КБРТ) '
      + 'FROM (Tromb INNER JOIN TrombDoza ON Tromb.ДатаТ=TrombDoza.ДатаТ) ' +
      'INNER JOIN (TrombComponents INNER JOIN TrombBrak ' +
      'ON TrombComponents.КодДТ = TrombBrak.КодКТ) ON ' +
      'TrombDoza.КодТ = TrombComponents.КодДТ ' +
