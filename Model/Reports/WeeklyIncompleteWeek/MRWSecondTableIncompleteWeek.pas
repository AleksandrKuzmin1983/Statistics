unit MRWSecondTableIncompleteWeek;
...
type
  IMRWSecondTableIncompleteWeek = interface
  ...
  TMRWSecondTableIncompleteWeek = class(TInterfacedObject, IMRWSecondTableIncompleteWeek)
  private
...
constructor TMRWSecondTableIncompleteWeek.create;
begin
...
procedure TMRWSecondTableIncompleteWeek.GetSQL(CSQL: String);
...
function TMRWSecondTableIncompleteWeek.GetProductWholeBlood: string;
begin
  TempSQL:=
...
function TMRWSecondTableIncompleteWeek.GetVolumeReinfusion: string;
begin
  TempSQL:=
...
function TMRWSecondTableIncompleteWeek.GetPreparedFitProductionErSuspFiltr: string;
begin
  TempSQL:=
...
function TMRWSecondTableIncompleteWeek.GetPreparedFitProductionErSusp: string;
begin
  TempSQL:=
...
function TMRWSecondTableIncompleteWeek.GetPreparedFitProductionSZP: string;
begin
  TempSQL:=
...
//
function TMRWSecondTableIncompleteWeek.GetOutingProductWholeBlood: string;
begin
  TempSQL:=
  ...
  GetSQL(TempSQL);
  if TempValue=0 then result:=' ' else result:=VarToStr(FormatFloat('0.000',TempValue));
end;

function TMRWSecondTableIncompleteWeek.GetOutingPreparedFitProductionErSuspFiltr: string;
begin
  TempSQL:=
  ...
  GetSQL(TempSQL);
  if TempValue=0 then result:=' ' else result:='Эр вз фил - ' + VarToStr(FormatFloat('0.000',TempValue));
end;

function TMRWSecondTableIncompleteWeek.GetOutingPreparedFitProductionErSusp: string;
begin
  TempSQL:=
    'SELECT Sum(Exped.ПГЭС)/1000 AS [Sum-ПГЭС] ' +
    'FROM Exped ' +
  ...
  GetSQL(TempSQL);
  result:=VarToStr(FormatFloat('0.000',TempValue));
  if TempValue=0 then result:=' ' else result:='Эр вз - ' + result;
end;

function TMRWSecondTableIncompleteWeek.GetOutingPreparedFitProductionSZP: string;
begin
  TempSQL:=
      ...
      'And (FactoryCal.День)>=Date()-Weekday(Date())-5));';
  GetSQL(TempSQL);
  if TempValue=0 then result:=' ' else result:='СЗП - ' + VarToStr(FormatFloat('0.000',TempValue));
end;
end.
