unit MRWCancellation_KrayIncomlpeteWeek;
...
uses
  ...
  UCheckNull;
type
  IMRWCancellation_KrayIncomlpeteWeek = interface
    function GetValueLitr(i: integer): string;
    function GetValueDoses(i: integer): string;
  end;

  TMRWCancellation_KrayIncomlpeteWeek = class(TInterfacedObject, IMRWCancellation_KrayIncomlpeteWeek)
  private
...
constructor TMRWCancellation_KrayIncomlpeteWeek.create;
var
  i: integer;
begin
...
function TMRWCancellation_KrayIncomlpeteWeek.GetValueLitr(i: integer): string;
begin
  if TempArray[i]=0 then result:=' ' else result:=VarToStr(FormatFloat('0.000',TempArray[i]/1000));
end;

function TMRWCancellation_KrayIncomlpeteWeek.GetValueDoses(i: integer): string;
begin
