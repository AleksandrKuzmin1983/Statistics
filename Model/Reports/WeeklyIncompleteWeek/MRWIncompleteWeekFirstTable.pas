unit MRWIncompleteWeekFirstTable;
...
uses
  ...
  UCheckNull;
type
  IMRWIncompleteWeekFirstTable = interface
  ...
  TMRWIncompleteWeekFirstTable = class(TInterfacedObject, IMRWIncompleteWeekFirstTable)
  private
...
constructor TMRWIncompleteWeekFirstTable.create;
var
  i: integer;
begin
  ...
  CSQL:=
    'SELECT [(Нужный) К еженедельному отчету план].ОЦК AS [Цельная кровь (план)], ' +
...
function TMRWIncompleteWeekFirstTable.GetValueLitr(i: integer): string;
begin
  if TempArray[i]=0 then result:=' ' else result:=VarToStr(FormatFloat('0.000',TempArray[i]/1000));
end;

function TMRWIncompleteWeekFirstTable.GetValuePlan(i: integer): string;
begin
  if TempArray[i]=0 then result:=' ' else result:=VarToStr(FormatFloat('0.000',TempArray[i]));
end;

function TMRWIncompleteWeekFirstTable.GetValuePercent(i: integer): string;
begin
  if TempArray[i]=0 then result:=' ' else result:=VarToStr(FormatFloat('0',TempArray[i]/10)) + '%';
end;

function TMRWIncompleteWeekFirstTable.GetValueTromboPacketsDoses(i: integer): string;
begin
