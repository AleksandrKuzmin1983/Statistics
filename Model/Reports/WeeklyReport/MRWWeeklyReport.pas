...
type
  ...
  private
    ...
    ReasonsForMedicalExemptions: IMRWReasonsForMedicalExemptions;
    Cancellation_Kray: IMRWCancellation_Kray;
    LPUConsultation: IMRWLPUConsultation;
...
begin
  ...
  frxTempReport.LoadFromFile(CurrentDir + '\BaseAdapters\Reports\WeeklyReport\WeeklyReport.fr3');
...
//Выдача в ЛПУ
  ...
  if not Assigned(ReasonsForMedicalExemptions) then
    ReasonsForMedicalExemptions := TMRWReasonsForMedicalExemptions.create;
  for i := 0 to 19 do
  begin
    frxTempReport.Variables.Variables['MedicalExemptions' + IntToStr(i+1)]:='''' + ReasonsForMedicalExemptions.GetValueName(i) + '''';
    if StrToInt(ReasonsForMedicalExemptions.GetValueNumber(i))=0 then frxTempReport.Variables.Variables['MedicalExemptionsNumber' + IntToStr(i+1)]:=''' '''
      else frxTempReport.Variables.Variables['MedicalExemptionsNumber' + IntToStr(i+1)]:='''' + ReasonsForMedicalExemptions.GetValueNumber(i) + '''';
    if StrToInt(ReasonsForMedicalExemptions.GetValueOuting(i))=0 then frxTempReport.Variables.Variables['MedicalExemptionsOutingNumber' + IntToStr(i+1)]:=''' '''
      else frxTempReport.Variables.Variables['MedicalExemptionsOutingNumber' + IntToStr(i+1)]:='''' + ReasonsForMedicalExemptions.GetValueOuting(i) + '''';
  end;
  if not Assigned(LPUConsultation) then
    LPUConsultation := TMRWLPUConsultation.create;
  frxTempReport.Variables.Variables['Transfusiolog']:='''' + LPUConsultation.GetTransfusiolog + '''';
  frxTempReport.Variables.Variables['Laboratory']:='''' + LPUConsultation.GetLaboratory + '''';
  frxTempReport.Variables.Variables['CheckLPU']:='''' + LPUConsultation.GetCheckLPU + '''';
  if not Assigned(Cancellation_Kray) then
    Cancellation_Kray := TMRWCancellation_Kray.create;
  for i := 0 to 31 do
    if (i<=7) or ((i>=16) and (i<=23)) then
      frxTempReport.Variables.Variables['Kray' + IntToStr(i+1)]:='''' + Cancellation_Kray.GetValueLitr(i) + ''''
    else
      frxTempReport.Variables.Variables['Kray' + IntToStr(i+1)]:='''' + Cancellation_Kray.GetValueDoses(i) + '''';
