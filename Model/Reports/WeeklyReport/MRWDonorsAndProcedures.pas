...
uses
  ...
  UCheckNull;
type
  IMRWDonorsAndProcedures = interface
    function GetValue(i: integer): string;
  end;

  TMRWDonorsAndProcedures = class(TInterfacedObject, IMRWDonorsAndProcedures)
  private
...
constructor TMRWDonorsAndProcedures.create;
var
  i: integer;
begin
  ...
  CSQL:=
    ...
    'And (NewOKDK.DateTap)>=Date()-Weekday(Date())-5)) AS [Не взято], ' +
...
function TMRWDonorsAndProcedures.GetValue(i: integer): string;
begin
