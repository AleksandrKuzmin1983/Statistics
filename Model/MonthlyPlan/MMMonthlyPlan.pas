...
type
  ...
  private
    ...
    ChangeDaysOfRecord: IMMChangeRecordMonthlyPlan;
...
begin
  ...
  begin
    ...
    if not Assigned(ChangeDaysOfRecord) then
      ChangeDaysOfRecord := TMMChangeRecordMonthlyPlan.create;
  ...
  begin
    ...
    begin
      ...
      for i:=0 to LBoxCalendar.GetItemsCount-1 do
        if LBoxCalendar.CheckSelected(i) then
          ChangeDaysOfRecord.AddRecordDays(Period, StrToDate(LBoxCalendar.GetItemsValue(i)));
      AfterSaving_Cancelling;
      GetStringGrid(CurrentForm);
      StringGrid.Visible(True);
